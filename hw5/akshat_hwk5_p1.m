...
% plot results
fig = figure;
sgtitle("HW5 P1a: Position trajectories with Optimal Control Gain Matrix");
subplot(3, 1, 1);
plot(tsteps, x_all(1, :), 'LineWidth', 2);
ylabel("$x$", 'Interpreter', 'latex', 'FontSize', 14);
subplot(3, 1, 2);
plot(tsteps, x_all(2, :), 'LineWidth', 2);
ylabel("$y$", 'Interpreter', 'latex', 'FontSize', 14);
subplot(3, 1, 3);
plot(tsteps, x_all(3, :), 'LineWidth', 2);
ylabel("$z$", 'Interpreter', 'latex', 'FontSize', 14);
xlabel("Time (s)");
saveas(fig, "figs/hw5p1a_pos.svg");

fig = figure;
sgtitle("HW5 P1a: Velocity trajectories with Optimal Control Gain Matrix");
subplot(3, 1, 1);
plot(tsteps, x_all(4, :), 'LineWidth', 2);
ylabel("$\dot{x}$", 'Interpreter', 'latex', 'FontSize', 14);
subplot(3, 1, 2);
plot(tsteps, x_all(5, :), 'LineWidth', 2);
ylabel("$\dot{y}$", 'Interpreter', 'latex', 'FontSize', 14);
subplot(3, 1, 3);
plot(tsteps, x_all(6, :), 'LineWidth', 2);
ylabel("$\dot{z}$", 'Interpreter', 'latex', 'FontSize', 14);
xlabel("Time (s)");
